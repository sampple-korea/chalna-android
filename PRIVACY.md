...
Last updated: 2026-08-10. This notice describes Chalna 1.2.0 behavior.
...
Settings and a Room index of Chalna-created capture metadata are stored locally. The index may include a local URI/reference, filename, creation time, duration, quality, audio flag, dimensions, size, favorite/trash state, export relationship, and playback position. Production Chalna has no Diagnostics or VisualLab collection surface. It does not collect assist structure, screenshots, foreground-app identity, accounts, location, contacts, communications, or advertising identifiers.
...
Recordings remain until deleted. Device Gallery items can be managed through Chalna, an authorized gallery/files app, or system storage tools. Vault items are managed through Chalna and disappear with app data/uninstall. Recently Deleted retains supported items for up to 30 days before purge. Local settings and the capture index remain until changed, cleared, or uninstalled. A stale index entry is removed when Chalna confirms that its referenced media no longer exists.

## Permissions

- Camera: required for video.
- Microphone: optional; required only when audio recording is enabled.
- Notifications: optional drawer visibility for ongoing capture, stop control, saved result, and errors. Denial does not block recording; Android still receives the required foreground-service notification call.
...
Privacy questions and suspected violations should be reported privately through the repository owner's GitHub Security Advisory channel.
