...
Missing legacy metadata is backfilled lazily. Durable metadata/export operations are consumed in bounded batches with capped retry delay, and existence reconciliation runs in small batches on relevant access. Visible thumbnail requests use target-sized `ContentResolver.loadThumbnail` work on `Dispatchers.IO`, cancellation when composition leaves the viewport, and an LRU keyed by the immutable content URI and requested size. Loading uses a quiet neutral surface rather than the launcher mark.

Selection supports select-all by an ID query, share, favorite, trash, restore, permanent delete, and Vault export. Batch work reports typed start/final/partial-failure events; successful items remain successful. Export relationships make the normal Vault-to-MediaStore copy idempotent. A missing exported copy clears the relationship before a new copy.
