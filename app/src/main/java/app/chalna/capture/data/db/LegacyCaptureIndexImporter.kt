...
class LegacyCaptureIndexImporter(
    ...
    suspend fun import(lastCapture: LastCapture?): LegacyImportResult = withContext(Dispatchers.IO) {
        database.migrationMarkerDao().byName(MARKER)?.let {
            return@withContext LegacyImportResult(it.importedRows, it.malformedRows, true)
        }
        val decoded = if (legacy.isFile) {
            runCatching { LegacyCaptureIndexCodec.decode(legacy.readText(StandardCharsets.UTF_8)) }
                .getOrDefault(app.chalna.capture.data.LegacyCaptureIndexSnapshot(malformedRows = 1))
        } else {
            app.chalna.capture.data.LegacyCaptureIndexSnapshot()
        }
