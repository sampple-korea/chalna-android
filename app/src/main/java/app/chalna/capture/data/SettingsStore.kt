...
class SettingsStore(
    ...
    val settings: StateFlow<CaptureSettings> =
        store.data
            .catch { failure ->
        ...
        store.data
            .catch { failure ->
                ...
                LastCapture(
                    ...
                    state =
                        p[LAST_STATE]?.let { runCatching { CaptureRecordState.valueOf(it) }.getOrNull() }
                            ?: CaptureRecordState.READY,
                ).takeIf(LastCapture::isUsable)
            }.stateIn(scope, SharingStarted.Eagerly, null)

    suspend fun update(value: CaptureSettings) =
        store.edit {
    ...
    suspend fun saveLastCapture(capture: LastCapture?) =
        store.edit {
            if (capture == null) {
                it.remove(LAST_URI)
