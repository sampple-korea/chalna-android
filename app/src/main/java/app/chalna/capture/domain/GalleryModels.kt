...
data class CaptureItem(
    ...
    fun isUsable(): Boolean = id.matches(OPAQUE_ID) && contentUri.startsWith("content://") &&
    ...
    companion object {
        ...
        fun from(capture: LastCapture): CaptureItem {
            val stableId = capture.id.takeIf { it.matches(OPAQUE_ID) } ?: StableCaptureId.from(
...
object GalleryFilter {
    fun apply(items: List<CaptureItem>, query: GalleryQuery): List<CaptureItem> {
    ...
    fun day(item: CaptureItem, zoneId: ZoneId): java.time.LocalDate =
        Instant.ofEpochMilli(item.createdAtMillis).atZone(zoneId).toLocalDate()
}
