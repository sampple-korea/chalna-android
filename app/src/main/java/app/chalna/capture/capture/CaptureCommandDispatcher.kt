...
class AndroidCaptureCommandDispatcher(
    ...
    private data class Projection(val kind: ProjectionKind, val stateVersion: Long)
    ...
    override fun dispatch(
        ...
        command: CaptureCommand,
        trigger: CaptureTrigger,
    ): CaptureCommandResult = synchronized(this) {
        ...
        if (resolved is CaptureCommandResult.AcceptedStart && attempts.hasAttempt()) {
            ...
            return@synchronized try {
                ...
                val failed = CaptureCommandResult.FailedToDispatch(
                    normalizedId,
        ...
        return@synchronized try {
            ...
            val rejected = CaptureCommandResult.FailedToDispatch(
                normalizedId,
                CaptureFailure(
                    code = if (Build.VERSION.SDK_INT >= 31 &&
    ...
    private fun resolve(
        ...
        val projectedStarting = projected == ProjectionKind.STARTING &&
        ...
        val command = when (requested) {
            CaptureCommand.TOGGLE, CaptureCommand.QUICK_TILE_TOGGLE -> when {
        ...
        return when (command) {
            CaptureCommand.START -> if (states.isAnonymousFinalizeGuardActive(id)) {
            ...
            CaptureCommand.STOP, CaptureCommand.AUTO_STOP, CaptureCommand.NOTIFICATION_STOP -> when (state) {
                is CaptureState.Recording -> CaptureCommandResult.AcceptedStop(id)
                is CaptureState.StartRequested, is CaptureState.StartingForeground,
                is CaptureState.OpeningCamera, is CaptureState.StartingRecorder -> CaptureCommandResult.AcceptedCancelStart(id)
                is CaptureState.CancelRequested, is CaptureState.StopRequested,
                is CaptureState.StoppingRecorder, is CaptureState.Finalizing -> CaptureCommandResult.AlreadyStopping(id)
    ...
    private fun CaptureCommandResult.requiresServiceDelivery(): Boolean = when (this) {
        is CaptureCommandResult.AcceptedStart,
        is CaptureCommandResult.AcceptedStop,
        is CaptureCommandResult.AcceptedCancelStart -> true
        else -> false
    }
